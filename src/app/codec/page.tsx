...
class EncoderFactory {
  ...
  static getAllEncoders(): { type: CodecType; encoder: Encoder }[] {
    return Object.entries(this.encoders).map(([type, encoder]) => ({
...
export default function CodecPage(props: CodecPageProps) {
  ...
  const codecOptions = EncoderFactory.getAllEncoders()
    .filter(({ type }) => type === 'base64' || type === 'url' || type === 'html') // 只显示已实现的
    .map(({ type, encoder }) => ({
      label: encoder.name,
      value: type
    }))
  ...
  const comingSoonOptions = EncoderFactory.getAllEncoders()
    .filter(({ type }) => type === 'base32' || type === 'base58' || type === 'base62')
    .map(({ type, encoder }) => ({
        ...
        <Section title="编码类型" description="选择要使用的编码格式">
          <div className={`p-4 flex flex-col gap-4`}>
            ...
            <div className={`text-sm text-gray-600 bg-gray-50 p-3 rounded-lg`}>
              <strong>{currentEncoder.name}:</strong> {currentEncoder.description}
              {!isImplemented && (
        ...
        <Section title="编码转换" description="输入文本进行编码或解码">
          <div className={`p-4 flex flex-col gap-4`}>
            ...
            <InputText
              ...
              hint={operation === 'encode' ? '输入要编码的原始文本' : '输入要解码的编码文本'}
            />
        ...
        <Section title="使用说明" description="各种编码格式的说明和使用场景">
          <div className={`p-4`}>
            <div className={`text-sm space-y-4`}>
              <div>
                <strong>Base64:</strong> 常用于数据传输、存储二进制数据为文本格式
              ...
              <div className={`text-gray-500`}>
                <strong>Base32:</strong> 相比Base64更易读，常用于二维码等场景（即将支持）
              </div>
              
              <div className={`text-gray-500`}>
                <strong>Base58:</strong> 比特币地址编码，避免易混淆字符（即将支持）
              </div>
