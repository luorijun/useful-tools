...
import {InputNumber, InputNumberProps} from '@/components/form/InputNumber'

export type FieldProxyProps = {
  component: 'inputEnum' | 'inputBool' | 'inputList' | 'inputForm' | 'inputFile'
...
export const Form = forwardRef(function Form<T extends Record<string, any>>(rawProps: FormProps<T>, ref: ForwardedRef<FormRef>) {
  ...
  if (props.fields) {
    const vEntries = props.fields.map(field => [
      field.props.name,
      field.props.rule ?? field.props.required ? z.any().refine(value => value !== undefined && value !== '', {
        message: '此项为必填项',
      }) : z.any(),
    ])
          ...
          switch (field.component) {
            case 'inputText':
              return <InputText
                ...
                onChange={(value) => {onChange(field.props.name, value)}}
              />
            case 'inputNumber':
              return <InputNumber
                key={index}
