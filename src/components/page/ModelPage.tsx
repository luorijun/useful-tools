...
import { type FieldProxyProps, Form } from "@/components/form/Form"
import { Page, type PageProps } from "@/components/page/Page"
import { Table, type TableColumn } from "@/components/Table"
import { useStatus, wrapHttpStatus } from "@/lib/hooks"
 ...
 * 模型页面
 */
export default function ModelPage<T extends Model>(
	rawProps: ModelPageProps<T>,
) {
	const props = {
						...
						builder: (model, index) => (
							<div className="flex gap-2 justify-center">
								{props.operations?.edit && (
									<EditModel<T> index={index} model={model} />
								)}
								{props.operations?.delete && (
									<RemoveModel<T> index={index} model={model} />
								)}
							</div>
						),
					})}
...
function AddModel() {
	...
	return (
		<div>
			...
			>
				<Form id={formId} fields={addFormFields} onSubmit={onSubmit} />
...
function EditModel<T extends Model>({
	...
	const onSubmit = async (value: Model) => {
		const { success } = await wrapHttpStatus(() => {
			return fetch(
				`${process.env.NEXT_PUBLIC_PROJECT_API_URL + props.api}/${modelId}`,
				{
					method: "PUT",
