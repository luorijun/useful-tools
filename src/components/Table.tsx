...
export function Table<T extends Record<string, any>>(props: TableProps<T>) {
	...
	return (
		<table className="w-full border border-separate border-spacing-0 rounded-lg">
			<thead>
				<tr className="h-10">
					{columns.map((column, index) => (
						<th
							key={index}
							className="px-4 border-r font-normal last:border-r-0"
						>
							{column.label}
						</th>
					))}
			...
			<tbody>
				{props.loading && <LoadData span={columns.length} />}
...
function LoadData({ span }: { span: number }) {
	return (
		<tr>
			<td
				colSpan={span}
				className="h-10 border-t text-center align-middle text-gray-500"
			>
				加载中...
			</td>
		</tr>
...
function EmptyData({ span }: { span: number }) {
	return (
		<tr>
			<td
				colSpan={span}
