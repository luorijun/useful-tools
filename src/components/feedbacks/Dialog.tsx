...
import {useStatus, wrapStatus} from '@/utils/status'
...
export function Dialog(props: DialogProps) {
  ...
  const onClose = function (e?: MouseEvent<HTMLElement>) {
  ...
  return !mounted ? null : createPortal(
    <div
      ...
      `}
    >
      <section className={`bg-white rounded-2xl shadow-2xl flex-none flex flex-col`} style={style}>
        <header className={`relative flex-none flex px-4 pt-4 items-center h-14`}>
          <h3 className={`text-xl`}>{props.title}</h3>
          {props.close && (
            <div className={`absolute top-4 right-4`}>
              <Button icon type={`danger`} onClick={onClose}>❌</Button>
            </div>
          )}
        </header>
        <div className={`p-4`}>
